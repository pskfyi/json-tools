...
import * as JsonPatch from "./mod.ts";
...
Deno.test("JsonPatch", async ({ step: t }) => {
  await t("classes", () => {
  ...
  await t(".apply()", async ({ step: t }) => {
    await t("applies a sequence of operations immutably", () => {
    ...
    await t("terminates when an invalid operation is encountered", () => {
      const patch: JsonPatch.Type = [
        new TestOp("", {}),
        new RemoveOp(""),
        new TestOp("", {}),
    ...
    await t("can remove a whole document", () => {
    ...
    await t("can remove then create a whole document", () => {
      const document = { foo: true };
      const patch: JsonPatch.Type = [
        new RemoveOp(""),
        new AddOp("", 999),
    ...
    await t("can overwrite a whole document", () => {
      const document = { foo: true };
    ...
    await t("doesn't mutated the patch", () => {
      const patch: JsonPatch.Type = [
        new AddOp("/A", {}),
        new AddOp("/A/B", 7),
      ];
